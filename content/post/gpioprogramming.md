...
4. **Using the BCM2835 board to control an LED via the GPIO pins** - provides the details, including code, for controlling an LED.
    * An advanced program is included that will demonstrate how to program a blinking LED by directly interfacing with the BCM2835's peripheral's registers.
...
If you don't have one, you'll need a Raspberry Pi. I used a Raspberry Pi 3B+ with the 'stretch' version of the Raspbian OS. Given that the Sunfounder Ultimate Starter Kit is advertised to work with a Raspberry Pi 4, I would expect the 4 series to work as well. I'm less sure about other Raspberry Pi versions, especially versions with 26 vs. 40 GPIO pins. See [How to setup a new Raspberry Pi from scratch](https://projects.raspberrypi.org/en/projects/raspberry-pi-setting-up) if you don't already have a Raspberry Pi and need help setting one up.
...
The primary SPI interface on the BCM2835, SPI0, is implemented on GPIO pins 7-11. Pins 7 & 8 are the 2 CE/CS pins available on the BCM2835. Pin 9 is MISO, 10 is MOSI, and 11 is the clock (SCLK). The BCM2835 has 2 auxilary SPI interfaces, SPI 1 (AKA AUX_SPI0) and SPI 2 (AKA AUX_SPI1). From the BCM2835 ARM Peripherals guide it looks like SPI 1 is available on pins 16-21 and SPI 2 on pins 35-39. These auxilary interfaces are available via the AUX I/O function.
...
> There is third way to interface with GPIO registers. In the case of SPI, there are 2 devices (generally availabe on many Raspberry Pi models), `/dev/spidev0.0` and `/dev/spidev0.1` via `ioctl()`. This method won't be covered in this article.
...
This next code snippet sets the offsets for the various types of registers (e.g., GPIO, PWM, SPI, etc). Notice that each offset starts at `bcm_peripherals` which was the result of a a `mapmem()` operation. This is the address space of the _I/O Peripherals_ block. The various register type offsets start at this offset. Then the base of each register type, e.g., `BCM_GPIO_BASE`, is added to the `bcm_peripherals` base address. Note that each offset is divided by `4`. Each of these offset variables, e.g., `bcm_gpio`, is a `uint32_t`. Note that pointer arithmetic is being performed. You may recall that pointer arithmetic operates on a variable based on the size of the type of the associated variable. For `unint32_t`'s their size is 4 bytes. So each pointer arithmetic operation works in multiples of 4 bytes. Adding 1 to a `uint32_t` type will increment it's value by 4. Now, with regard to adding the offsets of the various register types to `bcm_peripherals`, these offsets are specified in increments of 1, not 4. So for the pointer arithmetic to work as expected these offsets need to be divided by 4. Another way to think about what's going on here is to think of `someOffset/4` as the 32-bit offset from the register base address. So, for example, `BM_GPIO_BASE/4` is the number of 32-bit words that are offset from `bcm_peripherals`.
